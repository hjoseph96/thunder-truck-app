...
/**
 * Get remaining route coordinates from current position to destination
 * @param {Array} coordinates - Full route coordinates [{latitude, longitude}, ...]
 * @param {number} progress - Current progress along route (0-1)
 * @returns {Array} Remaining route coordinates from current position to end
 */
const getRemainingRouteCoordinates = (coordinates, progress) => {
  if (!coordinates || coordinates.length < 2) {
    return [];
  }
  ...
  // If at the end of the route, return empty array
  if (clampedProgress >= 1) {
    return [];
  }

  // Get current position and segment index
  const currentPosition = getCoordinateAtProgress(coordinates, clampedProgress);
  const segmentIndex = currentPosition.segmentIndex;

  // If we're at the last segment, return just the current position and final destination
  if (segmentIndex >= coordinates.length - 1) {
    return [currentPosition, coordinates[coordinates.length - 1]];
  }

  // Return current position plus all remaining coordinates
  const remainingCoordinates = [currentPosition, ...coordinates.slice(segmentIndex + 1)];

  return remainingCoordinates;
};

/**
 * Create polyline animation steps for smooth courier movement along route
 * @param {Array} coordinates - Route coordinates
 * @param {number} startProgress - Starting progress (0-1)
 * @param {number} endProgress - Ending progress (0-1)
 * @param {number} duration - Animation duration in milliseconds
 * @param {number} fps - Frames per second (default: 5fps for controlled performance)
 * @returns {Array} Array of animation steps with current position and remaining polyline
 */
const createPolylineAnimationSteps = (
  coordinates,
  startProgress,
  endProgress,
  duration = 3000,
  fps = 5, // Changed from 60fps to 5fps for controlled, resource-efficient animations
) => {
  ...
  for (let i = 0; i <= totalSteps; i++) {
    ...
    // Get courier position at this animation step
    const courierPosition = getCoordinateAtProgress(coordinates, currentProgress);

    // Get remaining polyline from courier position
    const remainingPolyline = getRemainingRouteCoordinates(coordinates, currentProgress);
...
/**
 * Interpolate between two progress values for smooth animation
 * @param {number} startProgress - Starting progress (0-1)
 * @param {number} endProgress - Ending progress (0-1)
 * @param {number} animationProgress - Animation progress (0-1)
 * @param {string} easing - Easing function type ('linear', 'easeInOut', 'easeOut')
 * @returns {number} Interpolated progress value
 */
const interpolateProgress = (startProgress, endProgress, animationProgress, easing = 'easeOut') => {
  ...
  switch (easing) {
    case 'easeInOut':
      ...
      break;
    case 'easeOut':
      easedProgress = 1 - Math.pow(1 - animationProgress, 2);
      break;
    case 'linear':
    default:
  ...
  return startProgress + (endProgress - startProgress) * easedProgress;
};
