...
/**
 * Get all user's carts using the fetchCarts query
 * @returns {Promise<Array>} Array of cart data
 */
export const fetchCarts = async () => {
  const result = await executeGraphQL(FETCH_CARTS);
  
  return result.fetchCarts || [];
};
...
/**
 * Remove a cart item using the removeCartItem mutation
 * @param {string} cartItemId - The ID of the cart item to remove
 * @returns {Promise<Object>} The removal result
 */
export const removeCartItem = async (cartItemId) => {
  try {
    const result = await executeGraphQL(REMOVE_CART_ITEM, {
      input: {
        cartItemId
      }
    });

    const removeResult = result.removeCartItem;
    ...
    if (!removeResult.success) {
      throw new Error(removeResult.message || 'Failed to remove cart item');
    }
