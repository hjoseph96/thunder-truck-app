...
/**
 * Fetch available spoken languages
 * @returns {Promise<Array>} Array of spoken language objects
 */
export const fetchSpokenLanguages = async () => {
  try {
    console.log('Fetching spoken languages...');
    
    const result = await executeGraphQL(FETCH_SPOKEN_LANGUAGES);
    console.log('Spoken Languages GraphQL Result:', result);
    
    const languages = result.fetchSpokenLanguages;
