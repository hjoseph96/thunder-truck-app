...
export class WebSocketService {
  ...
  requestCourierPosition() {
    ...
    try {
      ...
      if (this.send(positionRequest)) {
        console.log('📍 Courier position requested (polling)');
        return true;
      } else {
        console.error('❌ Failed to send position request');
        return false;
      }
    } catch (error) {
  ...
  startCourierPositionPolling(intervalMs = null) {
    if (this.positionPollingInterval) {
      console.log('📍 Courier position polling already active');
      return;
    }
    ...
    console.log(`📍 Starting courier position polling (every ${this.positionPollingIntervalMs}ms)`);
  ...
  stopCourierPositionPolling() {
    if (this.positionPollingInterval) {
      console.log('📍 Stopping courier position polling');
