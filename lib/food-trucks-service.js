...
export const fetchNearbyFoodTrucks = async ({
  ...
  perPage = 12
    ...
    const query = `
      query FetchNearbyFoodTrucks($params: FetchFoodTrucksInput!) {
        fetchFoodTrucks(params: $params) {
          foodTrucks {
            id
            name
            description
    ...
    // Build params object according to FetchFoodTrucksInput schema
    const params = {
      nearbyLat: latitude,
      nearbyLng: longitude,
      nearbyRadius: radius,
    ...
    console.log('fetchNearbyFoodTrucks - Sending params:', params);

    const variables = { params };

    const result = await executeGraphQL(query, variables);
    
    console.log('fetchNearbyFoodTrucks - GraphQL result:', result);
