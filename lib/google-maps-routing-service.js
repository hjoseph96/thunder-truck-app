...
export class GoogleMapsRoutingService {
  ...
  hasDeviatedFromRoute(currentLocation, routeCoordinates, progress) {
    ...
    // Import distance-based coordinate calculation
    const { getCoordinateAtProgress } = require('./animation-utils');

    // Find expected position using distance-based progress (same as demo)
    const expectedPosition = getCoordinateAtProgress(routeCoordinates, progress);
    ...
    // Use a more lenient threshold for demo (100 meters instead of 50)
    const threshold = ROUTE_CACHE_CONFIG.DEVIATION_THRESHOLD * 2;
