...
import { getCoordinateAtProgress } from './animation-utils';
/**
...
export const useCourierDemo = ({
  ...
  // Get current position based on step, using constant speed (distance-based)
  const getCurrentPosition = (step) => {
    ...
    // Use animation-utils helper for constant speed movement based on actual distance
    return getCoordinateAtProgress(routeCoordinates, routeProgress);
  };

  // Auto-start/stop demo based on enabled state
  useEffect(() => {
    if (enabled && routeCoordinates && routeCoordinates.length > 0 && !isActive && !isComplete) {
  ...
  }, [enabled, routeCoordinates, isActive, isComplete]);
