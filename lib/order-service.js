  ...
  return {
    ...
    formattedItems:
      order.orderItems?.map((item) => ({
        ...
        // Use id field for fetching additional details like imageUrl
        menuItemId: item.id,
      })) || [],
...
/**
 * Submits menu item reviews for an order
 * @param {string} orderId - The ID of the order
 * @param {Object} reviews - Object containing menu item reviews
 * @param {Object} reviews[menuItemId] - Review data for each menu item
 * @param {number} reviews[menuItemId].rating - Rating from 1.0 to 5.0
 * @param {string} reviews[menuItemId].comment - Optional comment
 * @returns {Promise<Object>} Submission result
 * @throws {Error} If submission fails
 */
export const submitMenuItemReviews = async (orderId, reviews) => {
  try {
    if (!orderId) {
      throw new Error('Order ID is required');
    }
    ...
    if (!reviews || typeof reviews !== 'object') {
      throw new Error('Reviews object is required');
    }

    const mutation = `
      mutation submitMenuItemReviews($orderId: ID!, $reviews: [MenuItemReviewInput!]!) {
        submitMenuItemReviews(orderId: $orderId, reviews: $reviews) {
          success
          message
          submittedReviews {
            menuItemId
    ...
    const reviewsArray = Object.entries(reviews).map(([menuItemId, review]) => ({
      menuItemId,
    ...
    if (response.errors && response.errors.length > 0) {
      throw new Error(response.errors[0].message || 'Failed to submit menu item reviews');
    }

    if (!response || !response.submitMenuItemReviews) {
      throw new Error('Invalid response format from menu item review submission');
    }

    return response.submitMenuItemReviews;
  } catch (error) {
    console.error('Error in submitMenuItemReviews:', error);
    throw error;
  }
};

/**
 * Submits a food truck review for an order
 * @param {string} orderId - The ID of the order
 * @param {Object} review - Food truck review data
 * @param {number} review.rating - Rating from 1.0 to 5.0
 * @param {string} review.comment - Optional comment
 * @returns {Promise<Object>} Submission result
 * @throws {Error} If submission fails
 */
export const submitFoodTruckReview = async (orderId, review) => {
  try {
    if (!orderId) {
      throw new Error('Order ID is required');
    }
    ...
    const mutation = `
      mutation submitFoodTruckReview($orderId: ID!, $rating: Float!, $comment: String) {
        submitFoodTruckReview(orderId: $orderId, rating: $rating, comment: $comment) {
          success
          message
          review {
            id
    ...
    if (response.errors && response.errors.length > 0) {
      throw new Error(response.errors[0].message || 'Failed to submit food truck review');
    }

    if (!response || !response.submitFoodTruckReview) {
      throw new Error('Invalid response format from food truck review submission');
    }

    return response.submitFoodTruckReview;
  } catch (error) {
    console.error('Error in submitFoodTruckReview:', error);
    throw error;
  }
};