import { executeGraphQL } from './api-service';
...
export const fetchOrder = async (orderId) => {
  try {
    ...
    if (!response.data || !response.data.fetchOrder) {
      throw new Error('Order not found or invalid response format');
    }

    return response.data.fetchOrder;
  } catch (error) {
...
export const fetchUserOrders = async (params = {}) => {
  try {
    ...
    if (!response.data || !response.data.fetchUserOrders) {
      throw new Error('Invalid response format from fetchUserOrders');
    }

    return response.data.fetchUserOrders;
  } catch (error) {
...
export const formatOrderForDisplay = (order) => {
  ...
  return {
    ...
    // Format order items
    formattedItems: order.orderItems?.map(item => ({
      ...item,
      price: item.totalPriceCents ? (item.totalPriceCents / 100).toFixed(2) : '0.00'
    })) || [],
    // Format order payments
    formattedPayments: order.orderPayments?.map(payment => ({
      ...payment,
      amountCharged: payment.amountChargedCents ? (payment.amountChargedCents / 100).toFixed(2) : '0.00'
    })) || [],
    // Format address
