...
export class Courier {
  constructor(id, name, currentLocation = null, route = null) {
    ...
    this.animationState = {
      ...
      animationDuration: 1500, // 1.5 seconds to match location update interval
  ...
  updateRouteProgress() {
    ...
    if (finalProgressDiff > SIGNIFICANT_CHANGE_THRESHOLD) {
      ...
      // This prevents race conditions since GPS updates every 1.5s match animation duration of 1.5s
      if (!this.animationState.isPolylineAnimating) {
        ...
        // Animation started - perfectly synchronized with 1.5s GPS interval
      } else {
  ...
  startWebSocketPositionPolling() {
    if (this.couriers.size > 0 && !webSocketService.isPollingActive()) {
      ...
      if (!webSocketService.isConnected()) {
        ...
        // Start polling with 1.5-second interval for more responsive tracking
        webSocketService.startCourierPositionPolling(1500);
      }
    }
  }
