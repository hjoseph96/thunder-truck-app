...
export class Courier {
  constructor(id, name, currentLocation = null, route = null) {
    ...
    this.animationState = {
      ...
      animationDuration: 3000, // 3 seconds to match location update interval for smooth continuous animation
  ...
  updateRouteProgress() {
    ...
    if (!this.totalRouteDistance) {
      ...
      console.log(
        `[DISTANCE-FIX] Total route distance calculated: ${this.totalRouteDistance.toFixed(2)}m for courier ${this.id}`,
      );
    }
    ...
    // Search range: ±200m from current distance position (handles GPS noise)
    const searchRangeMeters = 200;
    ...
    console.log(
      `[DISTANCE-FIX] Progress calculated: ${(closestProgress * 100).toFixed(2)}% (${closestDistanceAlongRoute.toFixed(1)}m/${this.totalRouteDistance.toFixed(1)}m) for courier ${this.id}`,
    );
...
export class CourierTrackingManager {
  ...
  updateCourierLocation(id, location) {
    ...
    console.log('[INFO] Location updated=>', location);
    ...
    if (!courier.route || courier.route?.length <= 2 || courier.checkRouteDeviation()) {
      console.info('[INFO] Route deviation detected, optimizing route...');
      this.requestRouteOptimization(id);
    }
  ...
  handleWebSocketLocationUpdate(data) {
    try {
      ...
      console.log('[INFO] Received location update:', data);
