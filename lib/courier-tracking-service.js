...
export class Courier {
  constructor(id, name, currentLocation = null, route = null) {
    ...
    this.totalRouteDistance = null; // Cache total route distance for distance-based progress
    ...
    this.animationState = {
      ...
      animationDuration: animationSettings.animationDuration, // Use configured animation duration
  ...
  setRoute(coordinates) {
    ...
    // Recalculate total route distance when route changes
    this.totalRouteDistance =
      coordinates && coordinates.length >= 2 ? calculatePolylineDistance(coordinates) : 0;
  ...
  updateRouteProgress() {
    ...
    // Calculate total route distance (cache it if not already calculated)
    if (!this.totalRouteDistance) {
      this.totalRouteDistance = calculatePolylineDistance(this.route);
    }
    ...
    // Find closest point on route using distance-based search
    const currentDistanceAlongRoute = currentTargetProgress * this.totalRouteDistance;

    // Search range: ±2000m from current distance position (handles large GPS jumps and route changes)
    const searchRangeMeters = 2000;
    const searchStartDistance = Math.max(0, currentDistanceAlongRoute - searchRangeMeters);
    const searchEndDistance = Math.min(
      this.totalRouteDistance,
      currentDistanceAlongRoute + searchRangeMeters,
    );

    let closestDistance = Infinity;
    let closestDistanceAlongRoute = currentDistanceAlongRoute;

    // Sample points along the search range to find closest point
    const sampleInterval = 10; // Sample every 10 meters
    for (let d = searchStartDistance; d <= searchEndDistance; d += sampleInterval) {
      const routePoint = getCoordinateAtDistance(this.route, d);
      const distance = calculateDistance(this.currentLocation, routePoint);

      if (distance < closestDistance) {
        closestDistance = distance;
        closestDistanceAlongRoute = d;
      }
    }

    // Fallback: If no close point found in search range, search the entire route
    if (closestDistance > 1000) { // If closest point is more than 1km away
      console.log('[INFO] Courier far from expected position, searching entire route');
      closestDistance = Infinity;
      closestDistanceAlongRoute = currentDistanceAlongRoute;
      
      // Search entire route with larger intervals for performance
      const fullSearchInterval = 50; // Sample every 50 meters for full route search
      for (let d = 0; d <= this.totalRouteDistance; d += fullSearchInterval) {
        const routePoint = getCoordinateAtDistance(this.route, d);
        const distance = calculateDistance(this.currentLocation, routePoint);
    ...
    // Convert distance back to progress (0-1)
    const closestProgress =
      this.totalRouteDistance > 0 ? closestDistanceAlongRoute / this.totalRouteDistance : 0;
    ...
    const MAX_BACKWARD_DISTANCE = 50; // meters - maximum allowed backward movement
    const MIN_FORWARD_DISTANCE = 5; // meters - minimum forward progress per update

    // Convert distance thresholds to progress ratios
    const MAX_BACKWARD_PROGRESS =
      this.totalRouteDistance > 0 ? MAX_BACKWARD_DISTANCE / this.totalRouteDistance : 0.01;
    const MIN_FORWARD_PROGRESS =
      this.totalRouteDistance > 0 ? MIN_FORWARD_DISTANCE / this.totalRouteDistance : 0.001;
    ...
    if (finalProgressDiff > SIGNIFICANT_CHANGE_THRESHOLD) {
      // PERFECT SYNC: Only start new animation if current one is complete
      // This prevents race conditions since GPS updates every 3s match animation duration of 3s
      if (!this.animationState.isPolylineAnimating) {
        this.startPolylineAnimation(currentTargetProgress, newTargetProgress, currentTime);
        // Animation started - perfectly synchronized with 3s GPS interval
      } else {
        // Animation in progress - queue the new target (will be handled when current completes)
        this.animationState.queuedTargetProgress = newTargetProgress;
      }
    }
  ...
  /**
   * Start polyline animation from start progress to end progress over 1.5 seconds
  ...
  startPolylineAnimation(startProgress, endProgress, startTime) {
    // FIX: Seamless animation chaining - if animation is already running,
    // start from current animated position instead of jumping to startProgress
    if (this.animationState.isPolylineAnimating && this.animationState.animationStartTime) {
      // Continue from current animated position to prevent visual jumps
      this.animationState.animationStartProgress = this.animationState.currentProgress;
    } else {
      // New animation - use provided start progress
    ...
    // Animation initialized seamlessly
  }
  ...
  updatePolylineAnimation(currentTime) {
    ...
    if (animationProgress >= 1) {
      ...
      console.log(`[INFO] CourierLocation: ${this.currentLocation.latitude}, ${this.currentLocation.longitude}`);
      console.log(`[INFO] Expected Position: ${getCoordinateAtDistance(this.route, this.animationState.currentProgress).latitude}, ${getCoordinateAtDistance(this.route, this.animationState.currentProgress).longitude}`);

      // Handle queued animation if one was waiting
      if (this.animationState.queuedTargetProgress !== undefined) {
        const queuedTarget = this.animationState.queuedTargetProgress;
        delete this.animationState.queuedTargetProgress;

        // Start queued animation immediately
        this.startPolylineAnimation(this.animationState.currentProgress, queuedTarget, currentTime);
...
export class CourierTrackingManager {
  constructor() {
    console.log(
      `[DISTANCE-FIX] CourierTrackingManager instance created at ${new Date().toISOString()}`,
    );
    ...
    this.lastLocationUpdates = new Map(); // Track last location update per courier to prevent duplicates
  ...
  updateCourierLocation(id, location) {
    ...
    if (!courier.route || courier.route?.length <= 2 || courier.checkRouteDeviation()) {
      this.requestRouteOptimization(id);
    }
   ...
   * Uses 5fps (200ms intervals) for controlled, resource-efficient animations
   */
  startAnimation() {
    ...
    const animate = () => {
      ...
      if (hasActiveAnimations) {
        ...
        // Use 5fps interval (200ms) instead of requestAnimationFrame for controlled performance
    ...
    // Start with 5fps interval (200ms)
    this.animationFrameId = setTimeout(animate, 200);
  }
  ...
  startWebSocketPositionPolling() {
    if (this.couriers.size > 0 && !webSocketService.isPollingActive()) {
      ...
      if (!webSocketService.isConnected()) {
        ...
        // Start polling with 3-second interval to match animation duration for smooth continuous movement
  ...
  handleWebSocketLocationUpdate(data) {
    try {
      ...
      // Deduplication: Check if this is a duplicate update within 100ms
      const lastUpdate = this.lastLocationUpdates.get(courier_id);
      const now = Date.now();
      if (lastUpdate && (now - lastUpdate.timestamp) < 100) {
        console.log('[INFO] Skipping duplicate location update within 100ms:', location);
        return;
      }

      console.log('[INFO] Received location update:', location);

      // Track this update to prevent duplicates
      this.lastLocationUpdates.set(courier_id, location);
