...
export class Courier {
  ...
  updateRouteProgress() {
    ...
    if (this.animationState.lastPosition) {
      distanceMoved = calculateDistance(
        this.animationState.lastPosition,
        this.currentLocation,
      );
    ...
    if (progressDiff < 0) {
      ...
      if (closestDistance > GPS_ACCURACY_THRESHOLD) {
        ...
        console.log(`🛑 Courier ${this.name}: GPS error detected (${closestDistance.toFixed(1)}m from route), maintaining progress ${(currentTargetProgress * 100).toFixed(1)}%`);
      } else if (Math.abs(progressDiff) > MAX_BACKWARD_PROGRESS) {
        // Large backward jump - constrain it
        newTargetProgress = Math.max(closestProgress, currentTargetProgress - MAX_BACKWARD_PROGRESS);
        console.log(`🔧 Courier ${this.name}: Large backward jump constrained from ${(closestProgress * 100).toFixed(1)}% to ${(newTargetProgress * 100).toFixed(1)}%`);
      } else {
        // Small backward movement within GPS noise - allow it
        newTargetProgress = closestProgress;
        console.log(`📍 Courier ${this.name}: Small backward movement allowed (GPS noise): ${(progressDiff * 100).toFixed(2)}%`);
      }
      ...
      if (distanceMoved > 5 && progressDiff < MIN_FORWARD_PROGRESS) {
        ...
        console.log(`⏩ Courier ${this.name}: Ensured minimum forward progress: ${(MIN_FORWARD_PROGRESS * 100).toFixed(1)}%`);
      } else {
        // Normal forward movement
        newTargetProgress = closestProgress;
        console.log(`✅ Courier ${this.name}: Forward progress: ${(progressDiff * 100).toFixed(2)}%`);
      }
    }
    ...
    if (finalProgressDiff > SIGNIFICANT_CHANGE_THRESHOLD) {
      this.animationState.targetProgress = newTargetProgress;
      this.animationState.lastUpdate = currentTime;

      console.log(`📍 Courier ${this.name}: Progress update ${(currentTargetProgress * 100).toFixed(2)}% → ${(newTargetProgress * 100).toFixed(2)}% (distance: ${closestDistance.toFixed(1)}m)`);
    }
  ...
  checkRouteDeviation() {
    ...
    // Use routing service to check deviation
    const hasDeviated = routingService.hasDeviatedFromRoute(
      this.currentLocation,
      this.route,
  ...
  async updateRouteToDestination() {
    ...
    try {
      ...
      const newRoute = await routingService.fetchRoute(
        this.currentLocation,
        this.destination,
        routingPreferences,
      );
  ...
  getRoutingPreferences() {
    ...
    return {
      profile: 'driving',
      exclude: ['ferry'], // Avoid ferries for faster delivery
      alternatives: false,
      steps: true,
      geometries: 'polyline',
      overview: 'full',
...
export class CourierTrackingManager {
  ...
  initializeRoutingService() {
    if (CURRENT_MAPBOX_CONFIG.PUBLIC_ACCESS_TOKEN && CURRENT_MAPBOX_CONFIG.ROUTING.enableRouting) {
      routingService.setAccessToken(CURRENT_MAPBOX_CONFIG.PUBLIC_ACCESS_TOKEN);
      console.log('Courier tracking: Mapbox Directions API initialized');
    } else {
      console.warn(
        'Courier tracking: Mapbox Directions API not configured, using fallback routing',
      );
    }
  }
  ...
  updateCourierLocation(id, location) {
    ...
    console.log(`📍 Updating location for courier ${courier.name}:`, location);
    ...
    console.log(`🔔 Notifying subscribers of location update for ${courier.name}`);
  ...
  startAnimation() {
    ...
    const animate = () => {
      ...
      this.couriers.forEach((courier) => {
        ...
        // Calculate time-based smooth interpolation for continuous movement
        const timeDelta = currentTime - animationState.lastUpdate;
        const progressThreshold = 0.0001; // More precise threshold for smoother animation

        if (
          Math.abs(animationState.targetProgress - animationState.currentProgress) >
          progressThreshold
        ) {
          // Enhanced smooth interpolation with time-based easing
          const diff = animationState.targetProgress - animationState.currentProgress;
          const smoothFactor = Math.min(0.15, timeDelta / 100); // Adaptive smooth factor based on time

          animationState.currentProgress += diff * smoothFactor;
          animationState.lastUpdate = currentTime;
          hasActiveAnimations = true;

          // Update courier status based on movement
          courier.status = Math.abs(diff) > 0.01 ? 'moving' : 'idle';
        } else {
          // Snap to target when very close to avoid infinite micro-movements
          if (Math.abs(animationState.targetProgress - animationState.currentProgress) > 0) {
